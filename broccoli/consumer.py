...
from broccoli.constants import WORKER_GREENLET
from broccoli.constants import WORKER_PROCESS
from broccoli.constants import WORKER_THREAD
from broccoli.constants import WORKER_TYPES
from broccoli.exceptions import ConfigurationError
from broccoli.exceptions import DataStoreGetException
from broccoli.exceptions import QueueException
from broccoli.exceptions import QueueReadException
from broccoli.exceptions import DataStorePutException
from broccoli.exceptions import QueueWriteException
from broccoli.exceptions import ScheduleAddException
from broccoli.exceptions import ScheduleReadException
from broccoli.registry import registry
...
class BaseProcess(object):
    def __init__(self, broccoli, utc):
    ...
    def enqueue(self, task):
        try:
            self.broccoli.enqueue(task)
        except QueueWriteException:
            self.broccoli.emit_task(EVENT_ERROR_ENQUEUEING, task, error=True)
...
class Worker(BaseProcess):
    def __init__(self, broccoli, default_delay, max_delay, backoff, utc):
        self.delay = self.default_delay = default_delay
        self.max_delay = max_delay
        self.backoff = backoff
        self._logger = logging.getLogger('broccoli.consumer.Worker')
        super(Worker, self).__init__(broccoli, utc)

    def loop(self, now=None):
        task = None
        exc_raised = True
        try:
            task = self.broccoli.dequeue()
        except QueueReadException:
            self.broccoli.emit_status(EVENT_ERROR_DEQUEUEING, error=True)
            self._logger.exception('Error reading from queue')
        except QueueException:
            self.broccoli.emit_status(EVENT_ERROR_INTERNAL, error=True)
        ...
        elif exc_raised or not self.broccoli.blocking:
            self.sleep()
    ...
    def process_task(self, task, ts):
        self.broccoli.emit_task(EVENT_STARTED, task, timestamp=to_timestamp(ts))
        self._logger.info('Executing %s' % task)
        start = time.time()
        try:
            try:
                self.broccoli.execute(task)
            finally:
    ...
    def requeue_task(self, task, ts):
        task.retries -= 1
        self.broccoli.emit_task(EVENT_RETRYING, task)
    ...
    def add_schedule(self, task):
        ...
        try:
            self.broccoli.add_schedule(task)
        except ScheduleAddException:
            self.broccoli.emit_task(EVENT_ERROR_SCHEDULING, task, error=True)
            self._logger.error('Error adding task to schedule: %s' % task)
        else:
            self.broccoli.emit_task(EVENT_SCHEDULED, task)

    def is_revoked(self, task, ts):
        try:
            if self.broccoli.is_revoked(task, ts, peek=False):
                return True
            return False
        except DataStoreGetException:
            self.broccoli.emit_task(EVENT_ERROR_INTERNAL, task, error=True)
            self._logger.error('Error checking if task is revoked: %s' % task)
            return True


class Scheduler(BaseProcess):
    def __init__(self, broccoli, interval, utc, periodic):
        super(Scheduler, self).__init__(broccoli, utc)
        ...
        if periodic:
            ...
            if not self._r:
                self._q -= 1
            self._counter = 0
        self._logger = logging.getLogger('broccoli.consumer.Scheduler')

    def loop(self, now=None):
        start = time.time()

        for task in self.broccoli.read_schedule(now or self.get_utcnow()):
    ...
    def enqueue_periodic_tasks(self, now, start):
        ...
        self.broccoli.emit_status(
            EVENT_CHECKING_PERIODIC,
            timestamp=self.get_timestamp())
        self._logger.debug('Checking periodic tasks')
        self._counter = 0
        for task in self.broccoli.read_periodic(now):
            self.broccoli.emit_task(
...
class Consumer(object):
    def __init__(self, broccoli, workers=1, periodic=True, initial_delay=0.1,
                 backoff=1.15, max_delay=10.0, utc=True, scheduler_interval=1,
                 worker_type='thread', check_worker_health=True,
                 health_check_interval=1):

        self._logger = logging.getLogger('broccoli.consumer')
        if broccoli.always_eager:
            self._logger.warning('Consumer initialized with broccoli instance '
    ...
    def start(self):
        if self.broccoli.always_eager:
            raise ConfigurationError(
                'Consumer cannot be run with broccoli instances where always_eager'
                ' is enabled. Please check your configuration and ensure that'
                ' "broccoli.always_eager = False".')
        # Log startup message.
        self._logger.info('Broccoli consumer started with %s %s, PID %s' % (
            self.workers,
            self.worker_type,
            os.getpid()))
