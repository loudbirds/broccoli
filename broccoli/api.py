...
from broccoli.exceptions import DataStoreTimeout
from broccoli.exceptions import QueueException
from broccoli.exceptions import QueueReadException
from broccoli.exceptions import QueueRemoveException
...
from broccoli.utils import local_to_utc, is_naive, aware_to_utc, make_naive, is_aware
from broccoli.utils import wrap_exception


class Broccoli(object):
    """
    Broccoli executes tasks by exposing function decorators that cause the function
    call to be enqueued for execution by the consumer.

    Typically your application will only need one Broccoli instance, but you can
    have as many as you like -- the only caveat is that one consumer process
    must be executed for each Broccoli instance.
    ...
    Example usage::
        ...
        # Create a broccoli instance and disable consumer-sent events.
        broccoli = RedisBroccoli('my-app', events=False)
        ...
        @broccoli.periodic_task(crontab(minute='0', hour='3'))
        def backup():
            # do a backup every day at 3am
            return
    """
    def __init__(self, name='broccoli', result_store=True, events=True,
    ...
    def get_storage(self, **kwargs):
        raise NotImplementedError('Storage API not implemented in the base '
                                  'Broccoli class. Use `RedisBroccoli` instead.')
    ...
    def emit_status(self, status, error=False, **data):
        if self.events:
            ...
            print("___emit_status___: {}".format(metadata))
...
class TaskResultWrapper(object):
    ...
    """
    def __init__(self, broccoli, task):
    ...
    def _get(self, preserve=False):
        ...
        if self._result is EmptyData:
            res = self.broccoli._get_data(task_id, peek=preserve)
    ...
    def revoke(self):
        self.broccoli.revoke(self.task)

    def restore(self):
        self.broccoli.restore(self.task)
...
class QueueTask(with_metaclass(QueueTaskMetaClass)):
    ...
    arbitrary data.  When enqueued with the :class:`Broccoli`, it will be
